...
public class CustomRefreshListView extends ListView implements AbsListView.OnScrollListener{
    ...
    private void initHeaderView() {
        headerView = View.inflate(getContext(), R.layout.customview_listview_top_onloadview, null);
        iv_arrow = (ImageView) headerView.findViewById(R.id.arrow);
        pb_rotate = (ProgressBar) headerView.findViewById(R.id.progress);
        tv_state = (TextView) headerView.findViewById(R.id.tip);
        tv_time = (TextView) headerView.findViewById(R.id.last_update_time);
    ...
    private void initFooterView() {
        if (footerView == null){
            footerView = LayoutInflater.from(context).inflate(R.layout.customview_listview_bottom_onloadview,null);
...
//        scrollState = i;
        //当滑动到底部时
        if (i == OnScrollListener.SCROLL_STATE_IDLE && absListView.getLastVisiblePosition() == absListView.getCount() - 1){
...
//        this.firstVisibleItem = firstVisibleItem;
        if (footerView != null){
            //判断此时加载的item完全显示  即 看到了最后一个item底部
            if (visibleItemCount == absListView.getCount()){
